...
const FALLBACK_JSON_FILE = path.join(
  CONFIG.FALLBACK_DIR,
...
const FORENSIC_RULES = {
  ...
  // === CANISTERWORM (CanisterWorm/TeamPCP supply chain worm) ===
  // deploy.js in scripts/ is the worm republishing tool: it queries npm's /-/whoami
  // endpoint with stolen tokens and republishes malicious code across victim packages.
  "scripts/deploy.js": {
    type: "CRITICAL",
    checkContent: true,
    campaign: "CANISTERWORM",
    indicators: [/\/\-\/whoami/i], // npm token-owner API — specific to the worm's propagation logic
    safePatterns: [],
  },
};
...
const FORENSIC_RULES_KEYS_LOWER = Object.keys(FORENSIC_RULES).reduce(
  (map, key) => {
...
const CRITICAL_PATTERNS = [
  ...
  {
    pattern:
      /curl\s+[^\s]{0,200}githubusercontent\.com\/[^\s|]{1,300}\|\s*(?:sh|bash|zsh)/i,
    desc: "Pipe raw GitHub content to shell",
    indicator: "REMOTE_CODE_EXEC",
  },
  {
    pattern:
      /wget\s+[^\s]{0,200}raw\.githubusercontent\.com\/[^\s|]{1,300}\|\s*(?:sh|bash|zsh)/i,
    desc: "Pipe raw GitHub content to shell",
    indicator: "REMOTE_CODE_EXEC",
  },
  {
  ...
  {
    pattern: /bun_environment/i,
    desc: "Shai-Hulud Payload",
    indicator: "SHAI_HULUD",
  },
  {
    pattern: /SHA1HULUD/i,
    desc: "Shai-Hulud Signature",
    indicator: "SHAI_HULUD",
  },
  {
  ...
  {
    pattern:
      /require\s*\(\s*["']child_process["']\s*\)\.\s*(?:exec|execSync|spawn|spawnSync)/i,
  ...
  {
    pattern: /-v\s+\/:\/host\b/i,
    desc: "Host mount in container",
    indicator: "PRIV_ESC",
  },

  // CanisterWorm (TeamPCP) — self-propagating npm supply chain worm (March 2026)
  // Uses stolen npm tokens + ICP blockchain C2 + systemd user persistence (pgmon service)
  {
    pattern: /tdtqy-oyaaa-aaaae-af2dq-cai[^\s]{0,50}icp0\.io/i,
    desc: "CanisterWorm ICP canister C2 identifier",
    indicator: "CANISTERWORM_C2",
    campaign: "CANISTERWORM",
  },
  {
    pattern: /raw\.icp0\.io/i,
    desc: "Internet Computer Protocol C2 channel (CanisterWorm)",
    indicator: "CANISTERWORM_C2",
    campaign: "CANISTERWORM",
  },
  {
    pattern: /systemctl\s+--user\s+(?:enable|start)\s+pgmon/i,
    desc: "CanisterWorm pgmon persistence service install",
    indicator: "CANISTERWORM_PERSISTENCE",
    campaign: "CANISTERWORM",
  },
  {
    pattern: /pgmon\.service/i,
    desc: "CanisterWorm pgmon service file reference",
    indicator: "CANISTERWORM_PERSISTENCE",
    campaign: "CANISTERWORM",
  },
  {
    pattern: /\/tmp\/pglog/i,
    desc: "CanisterWorm secondary payload drop path (/tmp/pglog)",
    indicator: "CANISTERWORM_PAYLOAD",
    campaign: "CANISTERWORM",
  },
  {
    pattern: /\.local\/share\/pgmon/i,
    desc: "CanisterWorm Python implant installation path",
    indicator: "CANISTERWORM_PERSISTENCE",
    campaign: "CANISTERWORM",
  },
];
...
const WARNING_PATTERNS = [
  {
    pattern: /http:\/\/[^\s"']{1,200}/,
    desc: "Unencrypted HTTP",
    indicator: "INSECURE_NETWORK",
  },
  {
    pattern: /\\x[0-9a-fA-F]{2}/,
    desc: "Hex-encoded string",
    indicator: "OBFUSCATION",
  },
  {
    pattern: /String\.fromCharCode/,
    desc: "Char code obfuscation",
    indicator: "OBFUSCATION",
  },
  {
    pattern: /atob\s*\(/,
    desc: "Base64 atob decode",
    indicator: "OBFUSCATION",
  },
  {
...
function setupSignalHandlers() {
  const handleShutdown = (signal) => {
    ...
    try {
      ...
      console.log(
        `${colors.red}    > Could not save partial report: ${e.message}${colors.reset}`,
      );
    }
...
function getUserInfo() {
  console.log(
    `${colors.cyan}[1/5] Identifying User Environment...${colors.reset}`,
  );
  const info = {
    gitName: "Unknown",
    gitEmail: "Unknown",
...
function loadFromCache(cacheFile, type) {
  try {
    ...
    const ageMinutes = Math.round(
      (Date.now() - fs.statSync(validated).mtimeMs) / 1000 / 60,
    );
    console.log(
...
function loadFromFallback(fallbackFile, type) {
  try {
    ...
    console.log(
      `    > ${type}: ${colors.yellow}Using offline fallback${colors.reset}`,
    );
    return content;
  } catch (e) {
    return null;
...
function saveToCache(cacheFile, content) {
  try {
    ...
    fs.writeFileSync(hashFile, computeHash(content), {
      encoding: "utf8",
      mode: 0o644,
    });
  } catch (e) {
...
async function fetchThreats(forceNoCache = false) {
  ...
  if (forceNoCache)
    console.log(
      `    > ${colors.yellow}Cache bypassed (--no-cache flag)${colors.reset}`,
    );
  ...
  try {
    ...
    // Source 3: CanisterWorm IOCs — loaded from the bundled CSV.
    // To update: download the latest CSV from socket.dev and replace
    // fallback/canisterworm-packages.csv, or pull the latest release of this repo.
    {
      const cwRaw = loadFromFallback(
        FALLBACK_CANISTERWORM_FILE,
        "CanisterWorm IOCs",
      );
      canisterwormData = cwRaw
        ? { status: "fulfilled", value: cwRaw }
        : {
            status: "rejected",
    ...
    if (wizData.status === "fulfilled" && wizData.value) {
      ...
      for (const [pkg, vers] of Object.entries(parsed)) {
        ...
        if (!campaignMap.has(pkg)) campaignMap.set(pkg, "SHAI_HULUD_2");
      }
      console.log(`    > [Source 1] Wiz.io: Loaded successfully.`);
    } else {
      const wizError =
        wizData.reason && wizData.reason.message
          ? wizData.reason.message
          : "No data";
      console.log(
        `${colors.red}    > [Source 1] Failed: ${wizError}${colors.reset}`,
      );
    }
    ...
    if (jsonData.status === "fulfilled" && jsonData.value) {
      ...
      const jsonError =
        jsonData.reason && jsonData.reason.message
          ? jsonData.reason.message
          : "No data";
      console.log(
        `${colors.red}    > [Source 2] Failed: ${jsonError}${colors.reset}`,
      );
    }

    // Process Source 3 (CanisterWorm IOCs — Socket.dev / Endor Labs)
    if (canisterwormData.status === "fulfilled" && canisterwormData.value) {
      const parsed = parseCanisterWormCSV(canisterwormData.value);
      for (const [pkg, vers] of Object.entries(parsed)) {
        if (!badPackages[pkg]) badPackages[pkg] = new Set();
        if (vers.length === 0) {
          badPackages[pkg].add("*");
      ...
      console.log(
        `    > ${colors.yellow}To update: download from https://socket.dev/supply-chain-attacks/canisterworm or pull the latest release of this repo.${colors.reset}`,
      );
    } else {
      const cwError =
        canisterwormData.reason && canisterwormData.reason.message
          ? canisterwormData.reason.message
          : "No data";
      console.log(
    ...
    console.log(
      `    > Total Threat Database: ${count} unique packages targeted.`,
    );
    return badPackages;
  } catch (e) {
    console.error(
      `${colors.red}Critical Error fetching feeds: ${e.message}${colors.reset}`,
    );
    return {};
  }
}
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            console.log(
              `    > ${sourceName}: HTTP ${res.statusCode}, trying fallback...`,
            );
            const fallback = loadFromFallback(fallbackFile, sourceName);
            if (fallback) resolve(fallback);
...
function parseWizCSV(data) {
  ...
  const result = {};
  const startIdx =
    lines[0] && lines[0].toLowerCase().includes("package") ? 1 : 0;
...
function parseCanisterWormCSV(data) {
  if (!data || typeof data !== "string") return {};
  ...
  const ecosystemIdx = headerCols.indexOf("ecosystem");
  const namespaceIdx = headerCols.indexOf("namespace");
  ...
  for (let i = 1; i < lines.length && i < 100000; i++) {
    ...
    // Ecosystem guard — only npm
    if (ecosystemIdx !== -1) {
      const eco = parts[ecosystemIdx]
        ? parts[ecosystemIdx].replace(/["']/g, "").trim().toLowerCase()
        : "";
      if (eco && eco !== "npm") continue;
    }

    const namespace =
      namespaceIdx !== -1 && parts[namespaceIdx]
        ? parts[namespaceIdx].replace(/["']/g, "").trim()
        : "";
    const name =
      nameIdx !== -1 && parts[nameIdx]
        ? parts[nameIdx].replace(/["']/g, "").trim()
        : "";
    const version =
      versionIdx !== -1 && parts[versionIdx]
        ? parts[versionIdx].replace(/["']/g, "").trim()
        : "";
  ...
  try {
    ...
    for (const [pkg, details] of Object.entries(json)) {
      ...
      if (
        ...
        result[pkg] = details.versions.filter(
...
function getSearchPaths() {
  ...
  // A. Active Global (NPM)
  try {
    const globalPrefix = execSync("npm root -g", {
      timeout: 10000,
      encoding: "utf8",
    }).trim();
  ...
  // E. Generic Caches
  const yCache = path.join(
    home,
    platform === "win32" ? "AppData/Local/Yarn/Cache" : ".cache/yarn",
  );
  ...
  const npmCache = path.join(
    home,
    platform === "win32" ? "AppData/Roaming/npm-cache" : ".npm",
  );
...
function scanDir(
  ...
  badPackages,
  depth = 0,
  maxDepth = CONFIG.DEFAULT_SCAN_DEPTH,
) {
  // Check shutdown flag
  if (isShuttingDown) return;
...
function checkPackageJson(pkgPath, pkgName, badPackages) {
  ...
  for (const [forensicPath, forensicRule] of FORENSIC_RULES_ENTRIES) {
    ...
    if (verification.confirmed) {
      ...
      const label = isCritical
        ? "CRITICAL: MALWARE FILE FOUND"
        : "SUSPICIOUS: Artifact Found";
      ...
      detectedIssues.push({
        ...
        campaign: forensicRule.campaign || campaignMap.get(pkgName) || "",
      });
    } else {
      ...
      detectedIssues.push({
        ...
        campaign: forensicRule.campaign || "",
      });
    }
  }
...
function verifySuspiciousFile(filePath, fileName) {
  ...
  if (rule.isJson) {
    try {
      ...
      if (
        rule.safeKeys &&
        rule.safeKeys.some((safeKey) => keys.includes(safeKey))
      ) {
      ...
      const hasSuspiciousKey = rule.requiredKeys.some((reqKey) => {
        ...
        if (
          keys.some((k) => k && k.toLowerCase().includes(reqKey.toLowerCase()))
        ) {
          return true;
        }
      ...
      if (hasSuspiciousKey) {
        return {
          confirmed: true,
          reason: "Contains suspicious JSON keys",
          severity: rule.type,
        };
      }
      return {
        confirmed: false,
        reason: "JSON structure benign",
        severity: rule.type,
      };
    } catch (e) {
...
function checkScripts(content, pkgName, pkgPath) {
  if (!content || !content.scripts || typeof content.scripts !== "object")
    return;
...
function checkLockfile(lockPath, badPackages) {
  ...
  const { content, error, size } = safeReadFile(
    lockPath,
    CONFIG.MAX_LOCKFILE_SIZE_BYTES,
  );
  if (error) {
...
function generateReport(userInfo, isPartial = false) {
  ...
  try {
    ...
    fs.writeFileSync(CONFIG.REPORT_FILE, csvContent, {
      encoding: "utf8",
      mode: 0o644,
    });
    console.log(`    > CSV saved to: ${CONFIG.REPORT_FILE}`);
...
async function uploadReport(csvContent, userInfo) {
  ...
  console.log(
    `${colors.yellow}    > Uploading Report to Security API...${colors.reset}`,
  );
  ...
  try {
    ...
    if (parsedUrl.protocol !== "https:") {
      console.log(
        `${colors.red}    > Upload Failed: Only HTTPS URLs are allowed${colors.reset}`,
      );
      return;
    }
  } catch (e) {
    console.log(
      `${colors.red}    > Upload Failed: Invalid API URL${colors.reset}`,
    );
    return;
  }
...
/**
 * Prints guidance for developers if CanisterWorm artifacts are detected.
 * Based on recommendations from Endor Labs, Socket Research, and security team advisories.
 */
function printCanisterWormAdvisory() {
  console.log(
    `\n${colors.red}╔══════════════════════════════════════════════════════════════════╗${colors.reset}`,
  );
  console.log(
    `${colors.red}║        ⚠  CANISTERWORM DETECTED — IMMEDIATE ACTION REQUIRED  ⚠  ║${colors.reset}`,
  );
  console.log(
    `${colors.red}╚══════════════════════════════════════════════════════════════════╝${colors.reset}`,
  );
  console.log(
    `\n${colors.yellow}This worm steals npm tokens and republishes malicious code across your${colors.reset}`,
  );
  console.log(
    `${colors.yellow}packages. It also installs a persistent Linux backdoor (pgmon service).${colors.reset}`,
  );
  console.log(
    `\n${colors.cyan}STEP 1 — Check for dropped payloads (Linux/macOS):${colors.reset}`,
  );
  console.log(`  ls -la /tmp/pglog /tmp/.pg_state`);
  console.log(`  ls ~/.config/systemd/user/pgmon.service`);
  console.log(
    `\n${colors.cyan}STEP 2 — Check if the backdoor is running:${colors.reset}`,
  );
  console.log(`  systemctl --user status pgmon`);
  console.log(
    `\n${colors.cyan}STEP 3 — REVOKE npm credentials (CRITICAL if tokens were present):${colors.reset}`,
  );
  console.log(
    `  Revoke all tokens at: https://www.npmjs.com/settings/<user>/tokens`,
  );
  console.log(
    `  Tokens in ~/.npmrc, /etc/npmrc, or env vars NPM_TOKEN/NPM_TOKENS are compromised.`,
  );
  console.log(
    `\n${colors.cyan}STEP 4 — Remove persistence (Linux):${colors.reset}`,
  );
  console.log(`  systemctl --user stop pgmon`);
  console.log(`  systemctl --user disable pgmon`);
  console.log(
    `  rm -f ~/.config/systemd/user/pgmon.service /tmp/pglog /tmp/.pg_state`,
  );
  console.log(`  rm -rf ~/.local/share/pgmon`);
  console.log(
    `\n${colors.cyan}STEP 5 — Clean your project dependencies:${colors.reset}`,
  );
  console.log(`  Remove malicious packages and run: npm install`);
  console.log(
    `\n${colors.cyan}STEP 6 — Check for unauthorized npm publishes (if you maintain packages):${colors.reset}`,
  );
  console.log(
    `  Check your npm account activity for unexpected patch releases.`,
  );
  console.log(`  Unpublish compromised versions and republish clean ones.`);
  console.log(
    `\n${colors.dim}  Reference: https://www.endorlabs.com/learn/canisterworm${colors.reset}`,
  );
  console.log(
    `${colors.dim}  Reference: https://socket.dev/blog/canisterworm-npm-publisher-compromise-deploys-backdoor-across-29-packages${colors.reset}\n`,
  );
}
