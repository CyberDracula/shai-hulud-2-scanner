...
test('scoped package (namespace present) — keyed as @namespace/name', () => {
  ...
  const result = parseCanisterWormCSV(csv);
  assertEqual(result['@bad-scope/bad-pkg'], ['2.0.0'], 'scoped name constructed correctly');
});
...
test('ecosystem column absent — all rows accepted', () => {
  ...
  const result = parseCanisterWormCSV(csv);
  assertEqual(result['evil-pkg'], ['1.0.0'], 'evil-pkg included without ecosystem column');
  assertEqual(result['another-pkg'], ['2.0.0'], 'another-pkg included without ecosystem column');
});
...
test('multiple distinct versions for the same package', () => {
  ...
  const result = parseCanisterWormCSV(csv);
  assertEqual(result['evil-pkg'], ['1.0.0', '1.0.1', '2.0.0'], 'all versions collected in order');
});
...
test('quoted fields — quotes stripped correctly', () => {
  const csv = cwCSV(['"npm","@scope","my-pkg","3.1.4","2026-01-01","2026-01-01"']);
  const result = parseCanisterWormCSV(csv);
  assertEqual(result['@scope/my-pkg'], ['3.1.4'], 'quotes stripped from all fields');
});
...
test('blank lines between data rows are ignored', () => {
  ...
  const result = parseCanisterWormCSV(csv);
  assertEqual(Object.keys(result).sort(), ['pkg-a', 'pkg-b'], 'both packages parsed, blanks skipped');
});
